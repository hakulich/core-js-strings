...
function sumOfCodes(str) {
  return str === undefined || str === null
    ? 0
    : str.split('').reduce((sum, value) => sum + value.charCodeAt(0), 0);
}
...
function isPalindrome(str) {
  ...
  const strWithoutSpace = str.toLowerCase().replaceAll(regex, '');
  const strReverse = strWithoutSpace.split('').reverse().join('');
  return strWithoutSpace === strReverse;
}
...
function findLongestWord(sentence) {
  ...
  const lengthWord = words.map((e) => e.length);
  const indexOfMax = Math.max(...lengthWord);

  return words.find((elm) => elm.length === indexOfMax);
}
...
function invertCase(str) {
  return str
    .split('')
    .map((elm) =>
      elm === elm.toLowerCase() ? elm.toUpperCase() : elm.toLowerCase()
    )
    .join('');
}
...
function extractNameFromTemplate(value) {
  ...
  return `${words[1]} ${words[2].substring(0, words[2].length - 1)}`;
}
